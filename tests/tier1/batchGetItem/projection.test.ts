...
describe('BatchGetItem — legal shared-prefix projections', { tags: ['batch', 'data-plane'] }, () => {
  ...
  // rejections: a target that over-applies the overlap rule fails here.
  const pk = 'proj-accept-bg'
  ...
  const batchGet = (expr: string, names: Record<string, string>) =>
    ddb.send(
  ...
  it('accepts sibling map paths (a.b, a.c) and returns both under the parent', async () => {
    const result = await batchGet('#a.#b, #a.#c', { '#a': 'a', '#b': 'b', '#c': 'c' })
  ...
  it('accepts two paths sharing a list index (l[0].x, l[0].y) and merges them into one element', async () => {
    const result = await batchGet('#l[0].#x, #l[0].#y', { '#l': 'l', '#x': 'x', '#y': 'y' })
    const items = result.Responses![hashTableDef.name]
    expect(items).toHaveLength(1)
    const l = items[0].l.L!
  ...
  it('accepts distinct list indices (l[0], l[1]) and returns both elements in order', async () => {
    const result = await batchGet('#l[0], #l[1]', { '#l': 'l' })
    const items = result.Responses![hashTableDef.name]
    expect(items).toHaveLength(1)
    const l = items[0].l.L!
